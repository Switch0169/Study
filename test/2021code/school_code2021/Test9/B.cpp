...
public:
    ...
    void Destroy();
    int Decode(const string condestr, char txtstr[]);
};
// 构建huffman树
...
void HuffMan::Coding()
{
    ...
    for(i = 1; i <= lnum; ++ i)
    {
        ...
        while(huffTree[j].parent != 0){
            int fa = huffTree[j].parent;
            if(j == huffTree[fa].leftchild){
                cd[start] = '0';
            }
            else{
                cd[start] = '1';
...
int main()
{
    ...
    for(i = 0; i < t; i ++)
    {
        ...
        for(int j = 0; j < K; j++){
            ...
            cin >> codestr;
            char txtstr[1000];
            if(myHuff.Decode(codestr, txtstr) == ok){
                cout << txtstr << endl;
            }
            else{
                cout << "error" << endl;
